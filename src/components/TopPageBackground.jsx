...
const TopPageBackground = ({direction, skewed, text, main, iconPosition, children, desc, background, color, opacity}) => {
    if (direction === 'left') {
        direction = 'right-0'
        skewed = 'skewed-left'
  ...
  return (
    <div className={`w-full h-[500px] absolute top-0 left-0 ${color} overflow-hidden z-[-1]`}>
      
      {/* SKEW */}
      <div className={`skewed ${skewed} ${opacity}`}></div>
      ...
      {/* BOTTOM SVG */}
      <img className='min-w-[700px] w-full absolute scale-y-[-1] bottom-[-1px] right-0 h-[50px] md:h-[100px] z-[3]' src={waveSVG} alt="" />
    </div>
  )
}
